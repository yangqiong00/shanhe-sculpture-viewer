...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowed = ['.obj', '.fbx', '.glb', '.gltf', '.stl'];
    const ext = path.extname(file.originalname).toLowerCase();
...
// Upload model file
app.post('/api/upload', authRequired, upload.single('model'), (req, res) => {
...
app.post('/api/sculptures', authRequired, (req, res) => {
  const { name, description, model_file, default_width, default_height, default_depth, unit } = req.body;
  if (!name || !model_file) {
    return res.status(400).json({ error: 'Name and model file are required' });
  }
  const result = db.prepare(
    'INSERT INTO sculptures (name, description, model_file, default_width, default_height, default_depth, unit) VALUES (?, ?, ?, ?, ?, ?, ?)'
...
app.put('/api/sculptures/:id', authRequired, (req, res) => {
  const { name, description, default_width, default_height, default_depth, unit } = req.body;
  db.prepare(
    'UPDATE sculptures SET name=?, description=?, default_width=?, default_height=?, default_depth=?, unit=? WHERE id=?'
  ).run(name, description || '', default_width, default_height, default_depth, unit || 'cm', req.params.id);
  const sculpture = db.prepare('SELECT * FROM sculptures WHERE id = ?').get(req.params.id);
  res.json(sculpture);
});
